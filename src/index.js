...
let username = document.getElementById("username-edit-field");
let usernameeditbtn = document.getElementById("username-edit-btn");
let currentPlayerMove = document.getElementById("show-current-player-move")
let move = true;
let moveForComputer = true;
let data = [];
let currentEditBtn;
let gridSize= 0;
...
let editUserName = (index)=>{
    if(username.value.length<=0){
        alert("please enter something")
    }
     else{
        whoIsPlaying[index].name = username.value
        username.value=""
     }
    ...
    document.querySelector("#username-edit-btn").addEventListener("click",()=>{
        editUserName(currentEditBtn)
        renderUserData(whoIsPlaying[0], whoIsPlaying[1]);
        document.getElementById("edit-container").classList.add("invisible")
...
const  showCurrentPlayerMove = (move) =>{
  currentPlayerMove.innerHTML=move
}
// Example usage
...
let renderUserData =  (player1,player2) =>{
  ...
  box-shadow: 0px 5px 10px 0px rgba(0,255,255,0.7);transition:all linear 0.4s">
       ...
       <div class="flex justify-end"><i class="ri-pencil-line" id="edit-pen1" onClick="functionfor1()"></i></div>
       ...
       <div class="flex justify-end"><i class="ri-pencil-line" id="edit-pen2" onClick="functionfor2()" ></i></div>
...
function createTicTacToeGrid(size) {
    ...
    for (let i = 0; i < size; i++) {
      ...
      for (let j= 0; j < size; j++) {
        ...
        cell.className = 'tttcell  border-gray-300 w-[3rem]  h-[3rem] md:w-[4rem] md:h-[4rem] xl:w-[5rem] xl:h-[5rem] flex items-center justify-center text-[3rem] cursor-pointer';
...
document.querySelector("#btn-1p").addEventListener("click",(e)=>{
    ...
    choice.style="display:none"
    document.querySelector(".moveshowercontainer").classList.remove("invisible");
...
const highlightLeadingPlayer  = () =>{
    if(user1.win>user2.win  && user1.win>computer.win && user1.win>0){
     document.querySelector(".player-1-row").style="background-color:#0B566A;"
     document.querySelector(".player-1-row").classList.add("rounded-full")
    }
    else if(user2.win>user1.win  && user2.win>computer.win && user2.win>0){
     document.querySelector(".player-2-row").style="background-color:#0B566A;"
     document.querySelector(".player-2-row").classList.add("rounded-full")
    }
    else if(computer.win>user1.win  && computer.win>user2.win && computer.win>0){
     document.querySelector(".computer-row").style="background-color:#0B566A;"
     document.querySelector(".computer-row").classList.add("rounded-full")
    }
}
...
const functionfor1 =()=>{
    ...
    document.getElementById("edit-container").classList.add("visible");
    document.getElementById("edit-container").classList.remove("invisible");
}
 const functionfor2=()=>{
    currentEditBtn=1
